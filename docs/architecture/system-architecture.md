...
⚠️ **`core` ではない。** `core` は**生成側と遊技側が共有するもの**を置く場所で、
遊技中の状態(選択・入力・メモ・取り消し)は**生成器が一切使わない**。
分かれ目は「生成器も使うか」である。使うなら `core`、遊技だけなら `web`。

**それでも React に依存させない。** 理由は 2 つ。

- **取り消し(undo)が自然に書ける。** action を積めば履歴になる
- **盤面の振る舞いを React 抜きでテストできる。** 描画のテストと分離できる

⚠️ **数独の規則(重複の検出・解法)を reducer に書かない。** それは `core` の責務である。
...
| 遊技者の入力が数独の規則に反する | エラーではない。⚠️ **何も出さない**(2026-08-06・[ADR 0004](../decisions/0004-feature-scope-from-client.md))。**盤面は入力をそのまま映す** |
