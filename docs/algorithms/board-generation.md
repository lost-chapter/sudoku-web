...
```
1. 完成盤を作る ──▶ 2. 穴をあける(一意解を保つ) ──▶ 3. 難易度を評価する
```
...
- **各ワーカーへ別々のシードを配る。** 同じシードを配ると同じ問題が並ぶ
- 生成結果はワーカーから集めて**決まった順序で**パックへ書く
  (**書き込み順を到着順にすると出力が非決定的になる**)

## 性能

公開されている値(いずれも C++ / Java の実装)。
...
⚠️ **同じ出典に「JavaScript 版は非常に遅い」と明記されている。**
本プロジェクトの生成器は TypeScript なので、**上の値をそのまま期待してはいけない。**
**工程 2 で必ず自前で実測し、[実装の進め方と現在地](../guides/implementation-roadmap.md) へ記録する。**
