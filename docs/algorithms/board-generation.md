...
`options.minimumClues` を指定すると、穴あけをその手がかり数以上で止められる。
省略時は従来どおり、消せるところまで消して極小の問題にする。
...
**「やさしい」は候補メモなしで解けることを要件にする。**
これは難易度クラスの判定(`rateDifficulty`)を変えるのではなく、
生成器が `easy` の問題を採用するときだけ追加する条件である。

- `minimumClues: 40` を指定し、手がかりを 40 個以上残す
- 評価手順が `Naked Single` だけの問題を採用する

手がかり数は難易度そのものではないが、この要望では**候補を書かずに進められる問題を
一定の手数に収めるための生成条件**として使う。`normal` 以降は従来の極小問題の生成を続ける。
...
そのうえで、候補メモを使わなくても次の一手を追いやすい問題を採用する。
今回の実装では `minimumClues: 40` とし、解法手順が `Naked Single` だけの問題に絞った。
パックを更新したときは、採用数・手がかり数・解法手数・手筋の分布を実測して記録する
([検証](../reports/2026-08-06-easy-without-notes.md))。
