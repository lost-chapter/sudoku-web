...
新しく採用する `easy` の問題は、次の条件を満たすものを優先する。

- Naked Single / Hidden Single の流れだけで最後まで進められる
- 候補メモを作って維持しなくても、盤面から次の一手を追える
- メモは補助として使えてもよいが、解答に必要な前提にはしない

これは**手がかり数や固定のスコアで判定する条件ではない**。
手がかり数と体感難易度の相関は低いため、具体的な手がかり数・再生成回数・採用率を
この文書で固定しない。実装側は、上の到達点を満たす問題をどう選ぶかを決め、
確認した結果を未測定のまま完了扱いにしない。
...
|-------------|--------------|---------------------|------|
| やさしい | `easy` | 1 | Single だけで解ける。**メモなしでも追いやすい問題を優先する** |
