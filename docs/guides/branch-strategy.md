...
| ブランチ | 役割 | 直接コミット |
|---------|------|------------|
| `main` | **リリース済みの状態。ここが GitHub Pages へ出る** | **禁止** |
| `develop` | 統合ブランチ。**動く状態を保つ** | **禁止**(下記) |
| `release/<版>` | **出す直前に固める場所**(下記) | **可**(管理役のみ) |
...
flowchart TB
  ...
  dev -->|リリースするとき| rel["release/x.y.z"]
  rel -->|タグを打つ| main["main"]
  rel -->|戻す(忘れやすい)| dev
  main -->|自動| pages["GitHub Pages"]
```
...
**`develop` から切り、`main` と `develop` の両方へ寄せてから消す。**
手順は [`ops-release` スキル](../../.claude/skills/ops-release/SKILL.md)。
...
| **固めている間も `develop` は動く** | 検査中に入った変更を巻き込んでしまう |

⚠️ **`release/` は「固める場所」であって「作る場所」ではない。**
入れてよいのは**版を上げるコミット・リリースノート・出す直前に見つかった修正だけ**。
新しい機能を足したくなったら `develop` へ入れて次の版に回す。

🔴 **`develop` へ戻すのを忘れない。** 版を上げたコミットが `develop` に無いと、
次のリリースで必ず競合する。**確かめ方は 1 つ。**

```bash
git log --oneline develop..main | cat     # 空であること
```

⚠️ **`main` からではなく `release/` からマージして戻す。**
`main` からだと、`main` にしか無いもの(過去のリリース)まで巻き込む。

⚠️ **片づけは `git branch -d`。`-D` を使わない。**
`-d` が拒否されたら「まだどちらかに入っていない」という意味であり、
それこそが見つけたかった寄せ漏れである。

## タグ

**`v<版>`**(`v0.1.0`)。**`main` の上で打つ。**
リリースブランチの上で打つと、そのブランチを消したときに何を指しているか追いにくくなる。
