...
**次の一手は工程 2 の 5(難易度の評価・agent-b)と工程 4 の残り(agent-c)。**
...
| # | 課題 | いつ分かるか |
|---|------|------------|
| 1 | 同梱パックを難易度ごとに何問にするか | **生成時間は判断材料にならない**(1,000 問が秒の単位)。**残るはファイルサイズと配信の観点**で、工程 2 の 6 でパックを書き出したときにバイト数を測って決める |
...
| 7 | 375px でセルが 44px を保てるか | ✅ **閉じた(2026-08-05)。達成不能と判明。**44px × 9 + 外枠 = **402px** 必要で 375px に入らない。⚠️ 先の「約 76px」はデバイスピクセルとの取り違えで、**CSS ピクセルでは 38.8px**(管理役が再実測)。**基準を 2 段に改訂**した —— 盤面のセルは **24px 以上**(WCAG 2.2 の最小)、入力パッドは **44px 以上**([画面構成と操作仕様](../ui/screens-and-interactions.md#押せる大きさ2026-08-05-に改訂)) |
