...
| ジョブ | 中身 |
|--------|------|
| 競合マーカーの残存 | マージの解決漏れを止める(2026-08-05 に管理役が 2 回踏んだので仕掛けにした) |
| 静的検査とテスト | Lint → 整形の確認 → 型チェック → テスト → 本番ビルド |
| **パックの再現性** | **`easy` 1,000 問は `diff` で並び順まで、`extreme` 200 問は集合として**収録済みと突き合わせる(2026-08-05・agent-b)。ワーカー数を収録時と変えて回すので、**並列度で中身が変わらないことも同時に見る**。管理役の手元で **5 秒**(3 ワーカー・`load average` 22) |
