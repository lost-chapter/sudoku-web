...
| 2 | **ブラウザのバーが伸縮したときの見え方**(`svh` の採用是非) | **ブラウザ面には URL バーが無く、`vh` = `dvh` = `svh` = `lvh` になる**。差を作れない。⚠️ **`svh` の採用は「同じマスを何度も見る遊びで寸法が動くのは高くつく」という理屈のままで、実測ではない**。実機では**バーが縮んだとき下に余白が出る**はず(管理役の手元でも 64px 余った) |

**⇒ 実機で触れる人がいれば、この 2 点だけ見てほしい。**
