...
| agent-b | 工程 2(`core` + `generator`) | 🔜 **2026-08-05 に着手指示済み。**最初の区切りは「盤面の表現 + 探索ソルバ(解 2 個で打ち切り)が通るところ」 |
| agent-c | 工程 3(`web`) | 🔜 **2026-08-05 に着手指示済み。**最初の区切りは「盤面が出てセルを選んで数字が入るところ」。手書きの 1 問で進める(b を待たない) |
