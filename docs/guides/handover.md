...
| agent-c | 課題 6(Mantine の部品ごと import) | 🔜 **次はここ。**いま JS gzip 106.65 KB / CSS gzip 34.31 KB |
| agent-b | 工程 5 の一部(CI でパックの再現性を検証) | ✅ **完了(2026-08-05)。**収録済みを作り直して `diff` するジョブを足した。**ワーカー数を収録時と変えて回すので、並列度の非依存も同時に見る** |
