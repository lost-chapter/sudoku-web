...
| agent-b | 工程 11 の 1(やさしい問題の質) | ⏳ **受け入れ条件を文書化済み。実装・パック更新・検証は未完了**。手がかり数だけで難易度を調整しない |
| agent-c | 工程 11 の 2〜3(セル内クリア・完成結果) | ⏳ **操作仕様と判定条件を文書化済み。実装・検証は未完了**。確定値クリア、メモクリア、手がかりセル、終了後を分ける |
| agent-d | 工程 11 の受け入れ条件・操作仕様の文書化 | ✅ **仕様を docs へ反映・コミット済み**。工程 11 の実装・パック更新・検証は未完了 |
