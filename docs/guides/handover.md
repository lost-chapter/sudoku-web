...
| agent-b | 工程 2 の 3〜4(完成盤の生成・穴あけ) | 🔜 **次はここ。**区切りは 3+4 をまとめて(3 だけでは測れるものが無いため。2026-08-05 に採用)。**乱数は `core` の外から注入する**([盤面の生成](../algorithms/board-generation.md#大前提-乱数は外から注入する)) |
