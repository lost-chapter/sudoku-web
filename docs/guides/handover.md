...
| agent-c | Playwright による E2E(キーボードの 2 点) | ⏳ **着手前。**コントラストの修正のあとに進める |
...
### ⚠️ 人の手でしか確かめられない 2 点(2026-08-05)

**agent-c と管理役の双方が、ブラウザ自動化からは検証できなかった。**
素の `<button>` でも同じだったので、**アプリ側ではなくツール側の制約**である。

| # | 確かめること | なぜ自動化で無理か |
|---|------------|------------------|
| 1 | **Enter / Space でボタンが起動するか** | 自動化から送るキーではネイティブの活性化が起きない |
| 2 | **モーダルを閉じたあとフォーカスが戻るか** | JS で開くと「開く前のフォーカス」が無く、Mantine の `returnFocus` を試せない |

**手で触れる人なら数秒で済む。**ここが通れば工程 4 の完了条件
(キーボードだけで最初から最後まで遊べる)を満たす。

🎯 **2026-08-05: Playwright で自動化する方針に決まった**(ユーザーの判断)。
**実ブラウザへ本物のキーイベントを送るので、上記 2 点はどちらも再現できる。**
agent-c が `packages/web` に E2E として実装中。**対象はこの 2 点だけに絞る**
(E2E を広げると維持の手間だけが増える)。
