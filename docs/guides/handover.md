...
| agent-c | コントラストの総ざらい | 🔁 **差し戻し中(2026-08-05)。**agent-c が 3 か所を直したが、**管理役の裏取りで矛盾の色に失格が 3 件残っていた**(下記)。修正待ち |
| agent-c | Playwright による E2E(キーボードの 2 点) | ✅ **完了・統合済み(2026-08-05)。3 件通過・3.7 秒。**管理役が CI ジョブにし、`returnFocus` をわざと切って落ちることも確認した |

### ⚠️ コントラストは「いちばん狭い画面」で測る(2026-08-05・管理役の裏取り)

**agent-c は幅 1280px で測って失格 0 と報告した。管理役が 375px で測り直したら 3 件出た。**

| 組み合わせ | 実測(375px) | 判定 |
|---|---|---|
| 矛盾の赤 × 同じ数字の強調(ライト) | **4.19** | ❌ |
| 矛盾の赤 × 選択中の背景(ライト) | **4.42** | ❌ |
| 矛盾の赤 × 同じ数字の強調(ダーク) | **4.34** | ❌ |

**原因は基準そのものが画面幅で変わることである。**
盤面の文字は幅に応じて拡縮するので、**1280px では 28px 太字 =「大きい文字」となり基準が 3:1 に緩む。**
375px では 16px になり **4.5:1** が要る。**広い幅だけで測ると、狭い幅の失格を必ず見落とす。**

⚠️ **測定器そのものにも穴が空きやすい。**管理役は最初、無効の判定を要素自身だけで見ており、
無効ボタンの中の `span.mantine-Button-label` を「有効で失格」と誤判定した。
**無効かどうかは `closest("[disabled],:disabled")` で祖先まで辿る。**
