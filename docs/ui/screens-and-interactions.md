...
| `0` / `Backspace` / `Delete` | **選択中のセルに入れた確定値をクリアする**。メモはこの操作で消さず、確定値を消しても自動で復元しない |
| 同じ数字をもう一度 | **消す**(トグル) |
| 手がかりのセルへの確定値入力・確定値クリア | **何も起きない**。エラーも出さない |

**「消す」は確定値を対象にする。** セル内の候補メモを消す操作とは別であり、
確定値のクリアでメモを一緒に消したり、以前のメモを復元したりしない。
...
| 操作 | 動き |
|------|------|
| セルに触れる(タップ開始) | そのセルを選択する。短いタップではガイドを表示しない |
| 約 180ms 押し続ける | タップしたセルの中心を基準に `1 2 3 / 4 5 6 / 7 8 9` の大きなエリアガイドを表示する。開始時は中央エリアの **5** を強調する |
| 指を動かす | 方向ではなく、指が入っているエリアで数字を判定する。選択中のエリアを青く強調し、指の近くに **現在の数字を大きく表示するガイド**を出す |
| 指を離す | 指を離したエリアの数字を選択中のセルへ入力する。メモモード中はメモとして扱う。ガイドの外で離した場合は取り消す |
...
操作履歴を持つ場合も、確定値クリアとは別の操作として扱い、消したメモだけを戻せるようにする。
