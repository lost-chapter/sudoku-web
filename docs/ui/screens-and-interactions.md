...
| 画面 | 役割 |
|------|------|
| ホーム | 難易度を選ぶ。**難易度ごとに正解した問題数を表示する**。遊びかけがあれば「続きから」を出す。⚠️ **遊びかけを開くのは「続きから」を選んだときだけ**(2026-08-05 決定。起動時に必ず開くと、別の難易度で遊び始められない) |
| ゲーム | 盤面を遊ぶ。**アプリの本体で、ここに時間の大半を使う** |
| 設定 | テーマ・補助表示の切替。ゲーム画面からモーダルで開く |
| 完成 | 自分で盤面を埋め終わったときに出す。結果として「正解」または「不正解」を表示し、次の問題へ進む導線 |
...
flowchart LR
  ...
  game -->|盤面を埋め終える| done["完成結果<br/>正解 / 不正解"]
...
| `0` / `Backspace` / `Delete` | **選択中のセルに入れた確定値と候補メモをクリアする**。手がかりセルは変えない |
| 同じ数字をもう一度 | **消す**(トグル) |
| 手がかりのセルへの確定値入力・確定値クリア | **何も起きない**。エラーも出さない |

**「消す」は確定値と候補メモを対象にする。** 「セル内クリア」は候補メモだけを対象にする別操作であり、
確定値を消したあとに消した候補メモを自動で復元しない。

### セル内クリア

**セル内クリアは、選択中の 1 セルにある候補メモをすべて消す操作である。**
確定値クリアと同じ操作へまとめず、利用者に「セル内クリア」と分かる名前で提供する。
キー・ボタン・ジェスチャーのどれを入口にするかは実装側で決めてよいが、次の意味は変えない。

| 状態 | セル内クリアの動き |
|------|-------------------|
| 候補メモがあるセル | **そのセルのメモだけをすべて消す**。他のセルと確定値には触れない |
| 確定値が入っているセル | **確定値を消さない**。候補メモが無ければ何も起きない |
| 手がかりセル | **何も起きない**。手がかりを変更しない |
| 正解・あきらめの後 | **何も起きない**。終了した盤面を変更しない |
| 不正解の表示中 | **盤面を直せる**。誤答を修正して再判定できる |
...
| 確定入力を消す | **確定値と候補メモを一緒に消す**(取り消しで戻す) |
...
(モードは変わらない)。フリック判定中は、キーの上に iPhone 風の数字ガイドを表示する。
[検討の記録](../reports/2026-08-06-flick-input-survey.md)。
...
#### スマホのセルからスワイプして数字を入力(2026-08-06)

スマートフォンでは、**空きセルまたは入力済みセルから指を動かして数字を 1 手で入力できる**。
既存のテンキー・メモのスイッチ・キーボード操作は残す。

| 操作 | 動き |
|------|------|
| セルに触れる(タップ開始) | そのセルを選択する。ガイド表示前の短いタップでは数字を入力しない |
| 約 180ms 押し続ける | **セルフリック入力モード**に入り、タップしたセルの中心を基準に `1 2 3 / 4 5 6 / 7 8 9` の大きなエリアガイドを表示する。開始時は中央エリアの **5** を強調する |
| 指を動かす | 方向ではなく、指が入っているエリアで数字を判定する。選択中のエリアを青く強調し、指の近くに **現在の数字を大きく表示するガイド**を出す |
| 指を離す | セルフリック入力モードでは、指を離したエリアの数字を選択中のセルへ入力する。メモモード中はメモとして扱う。ガイドの外で離した場合は取り消す |
| セルフリック入力モードでその場で指を離す | **中央エリアの 5 を選択中のセルへ入力する**。メモモード中は 5 の候補メモを立てる / 落とす |
| ガイド表示前にタップを終える | 数字は入力せず、セルの選択だけを確定する |
| 手がかりのセル / 完成後の盤面 | スワイプ入力を受け付けない |

**タップしたセルを基準に少し遅れてエリアを表示し、タップ終了時の位置で確定する。** 方向ベクトルや角度ではなく、表示中の 3×3 エリアそのものを判定領域にする。
文字選択・コンテキストメニュー・画面スクロールと
競合させないため、スマートフォンのゲーム画面では `touch-action: none`、
`user-select: none`、`overscroll-behavior: none` を使う。
画面端の OS ジェスチャがアプリより優先される場合は、OS の動作を上書きしない。
...
**セル内クリアも 1 セルを対象とする 1 操作として扱う。**
操作履歴を持つ場合も、「消す」とは別の操作として扱い、セル内クリアでは消したメモだけを戻せるようにする。

### 盤面を埋め終えたときの判定

**盤面の空きがなくなった時点で、遊技者が入力した盤面と解を比較する。**
途中の入力では結果を表示しない。

| 条件 | 完成結果 | その後 |
|------|----------|--------|
| すべてのセルが解と一致する | **「正解」** | 終了。次の問題へ進める |
| 空きはないが、1 つでも解と違うセルがある | **「不正解」** | 不正解を表示し、数字を直せる |
| まだ空きがある | 結果を表示しない | そのまま遊技を続ける |

**誤答は、行・列・ブロックの重複を表示しなくても「不正解」として扱う。**
解と違うセルが 1 つでもあれば「正解」にしてはならない。
結果は目で読める文言で表示し、読み上げにも同じ意味が届くようにする。

#### 手がかりセルと終了後の制約

| 状態 | 盤面への操作 |
|------|-------------|
| 手がかりセルを選択中 | そのセルへの確定値入力・確定値クリア・メモ入力・セル内クリアは**何も起きない**。手がかりの値は変えない |
| **正解・あきらめの後** | **盤面への入力・クリア・メモ・取り消し・やり直し・選択移動を受け付けない**。許可するのは「次の問題へ」などの画面遷移だけ |
| **不正解の表示中** | **盤面への入力・クリア・メモ・選択移動を受け付ける**。誤答を直して再び全埋めにできる |
...
| そのあと | **入力できない**(遊技が終わった状態)。解答盤面を見られる状態で「次の問題へ」に進める |
...
| 結果の案内 | 確定後は確認モーダルを閉じ、ゲーム画面上部に**「答えを表示しました」と「次の問題へ」を出す**。モーダルで解答盤面を覆わない |
...
### 🔴 「完成結果」と「あきらめ」を同じ判定で見ない(2026-08-06)

**あきらめると盤面は解と一致する**。そのため**盤面が解と一致するかだけで完成結果を決めると、
諦めた遊技者に「正解」や「完成しました」と知らせてしまう。**

| 判定 | 意味 |
|------|------|
| **盤面が解と一致するか** | あきらめても真。自力で埋めた結果が不一致なら偽 |
| **遊技者が自分で解き終えたか** | あきらめたら偽。**「正解」は自力で埋めて一致したときだけ出す** |
| **あきらめた状態** | **完成結果ではない**。解を表示しても「正解」にはしない |
...
✅ **現在は、あきらめの確定後に確認モーダルを閉じ、解答盤面をそのまま見られるようにした。**
「答えを表示しました」と「次の問題へ」はゲーム画面上部に残し、正解・不正解の完成結果とは分けている。

### 正解結果の保存(2026-08-06)

**自分で正解した問題だけを `localStorage` に記録する**。あきらめて解を表示した問題は記録しない。
問題パックのパス・行番号・配信版で問題を特定し、同じ問題をもう一度解いても二重に数えない。
ホーム画面の難易度カードには、**正解した数 / 収録数**を表示する。

⚠️ **同梱の退避問題は結果を記録しない**。配信パック内の問題 ID を持たないためである。
...
| **正解した進み具合が分からない** | 正解数を保存していない | **各カードに「正解した数 / 収録数」を表示する** |
...
| **縦向きの盤面が画面の端から近すぎる** | 12px | **16px。**⚠️ **OS の画面端ジェスチャとも重なっていた** |
...
### ⚠️ 横向きの盤面は端から 8px にする

**縦向きは 16px、横向きは 8px にする。**
⚠️ **横向きは盤面の取り分がいちばん厳しいので、余白は最小限に広げる。**

⚠️ **画面端の OS ジェスチャとは重なったままである。**
**⇒ 盤面からの数字スワイプを追加したため、盤面自身は `touch-action: none` で操作を受け取る。**
左右端の OS ジェスチャがアプリより優先される可能性は残るので、
**端から始めた操作まで OS より優先させることはしない。**
...
### 高さが足りない端末でもスクロールさせない(2026-08-06・追加指定)

**スマートフォンでは高さに関係なく、ページも盤面の領域もスクロールさせない。**
高さが足りない場合は盤面を残りの領域へ縮める。したがって、極端に短い表示領域では
セルの 24px 下限を維持できないことがあるが、**今回の優先順位はスクロールを発生させないこと**である。

| 状態 | 見せ方 |
|------|--------|
| 通常のスマートフォン | 1 画面に収める。セルは可能な限り大きくする |
| 高さが特に短いスマートフォン | 盤面を縮める。ページ・盤面領域ともスクロールさせない |
...
各カードには `localStorage` に記録した**その難易度の正解数 / `manifest.json` の収録数**を表示する。
正解数は問題パックのパス・行番号・版で重複を除く。あきらめた問題は含めない。
...
- [システム構成](../architecture/system-architecture.md) — 状態管理・進行・正解結果の保存
