...
### ⚠️ 経過時間は出さない(2026-08-06 に削除)

**発注者の要望で、タイマーとスコアの類は一切持たない。**
表示だけでなく、**計測も保存もしない**(`elapsedMs` を進行の保存から外した)。

以前は「盤面のすぐ上・右寄せ」に置くと決めて実装していたが、
**機能ごと無くなったので位置の議論も無効である。**
...
### ⚠️ 間違いを教えない(2026-08-06 に削除)

**発注者の要望で、次の 3 つを設定ごと消した。**

| 消したもの | 何をしていたか |
|-----------|--------------|
| 矛盾の表示 | 同じ行・列・ブロックで数字が重複したら印を付けていた |
| 残り数の表示 | 各数字があと何個入るかを出し、0 になった数字をパッドで落としていた |
| 誤りの即時指摘 | 解と違う入力をその場で指摘していた |

**「規則の重複」も出さない。** 解を見る「誤り」だけでなく、
**数独の規則だけを見る「矛盾」も消す**という判断である(2026-08-06・ユーザー)。

⚠️ **読み上げのラベルからも「重複」「誤り」を外した。**
画面に印を出さないのに読み上げだけ残すと、**見える人と見えない人で分かることが変わる。**

**`core` の `findConflicts` は残っている。** 盤面ロジックとしては正しく、
あとで要ると言われる可能性がある。**web から呼ばないだけ**である。
